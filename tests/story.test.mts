...
/* ============================================================
   人称の決めつけ（2026-09-13 実機で発見）
   ============================================================
   本人は性別を話していないのに、シーンが「彼」と書いていた。
   しかも同じ画面のすぐ下で性別を質問していた。
   「まだ見えていないもの」とシーンが矛盾したのと同じ構造。
*/

test("checkFragment : 実機で出た「彼」の文を弾く", () => {
  // ⚠ これは本番で実際に出た文（2026-09-13）。短くせずそのまま残すこと
  const real =
    "夜が深まり、静まり返った空間の中、自分だけの時間を味わうために選んだのは" +
    "夜中まで続く映画の世界。ネトフリでお気に入りの映画を見ながら、" +
    "ソファーベッドはそのまま、贅沢なシアターに変わる。" +
    "画面の中の日常に潜むドラマチックな瞬間が、彼の心に小さな灯火をともす。" +
    "何気ない日々の中、それこそが贅沢であり、特別な時間。" +
    "そのひとときが織りなす贅沢感に包まれて、次第に夢の中へと沈んでいく。";
  const r = checkFragment(real, "夜中までネトフリで好きな映画を見ながらソファーベットで寝落ちする");
  assert.equal(r.ok, false);
  assert.equal(r.reason, "asserted_person");
  assert.equal(r.detail, "彼");
});

test("checkFragment : 主語を置かなければ通る", () => {
  const fixed =
    "夜が深まり、静まり返った空間の中、自分だけの時間を味わうために選んだのは" +
    "夜中まで続く映画の世界。ネトフリでお気に入りの映画を見ながら、" +
    "ソファーベッドはそのまま、贅沢なシアターに変わる。" +
    "画面の中の日常に潜むドラマチックな瞬間が、心に小さな灯火をともす。" +
    "何気ない日々の中、それこそが贅沢であり、特別な時間。" +
    "そのひとときが織りなす贅沢感に包まれて、次第に夢の中へと沈んでいく。";
  assert.equal(checkFragment(fixed, "夜中までネトフリで好きな映画を見ながらソファーベットで寝落ちする").ok, true);
});

test("checkFragment : 彼女・男性・女性も弾く", () => {
  const base = "あ".repeat(200);
  for (const w of ["彼女は", "男性が", "女性の", "少女は"]) {
    const r = checkFragment(base + w + "そこにいた。", "");
    assert.equal(r.ok, false, w);
    assert.equal(r.reason, "asserted_person", w);
  }
});

test("checkFragment : 「彼女」を「彼」と誤って報告しない", () => {
  const r = checkFragment("あ".repeat(200) + "彼女は笑っていた。", "");
  assert.equal(r.detail, "彼女");
});

test("checkFragment : 「彼ら」「彼方」は本人を指さないので通す", () => {
  for (const w of ["彼らの声が遠くにあった。", "彼方に灯が見えた。"]) {
    assert.equal(checkFragment("あ".repeat(200) + w, "").ok, true, w);
  }
});
